...
#                                                                              **self.db_config)
#
#     def handle_classifications_data(self, data, address, data_handler):
#         try:
#             timestamp_data = datetime.datetime.now()
#             prediction = int.from_bytes(data, byteorder='big')
#             prediction_label = data_handler.class_labels[prediction]
#             data_handler.insert_classifications_data(self.node_id, timestamp_data, prediction_label)
#         except IndexError:
#             print("接收到的数据越界：", data)
#         except Exception as e:
#             print(f"解析接收到的数据时出错: {e}")
#
#     def receive_data_frame(self, client_socket, address, data_handler):
...
#             if node_data:
#                 data_handler.insert_node_data(node_data)
#                 print("节点数据插入成功：", node_data)
#
#     def receive_classifications_data(self, client_socket, address, data_handler):
...
#                 print(f"{address[0]}:{address[1]}断开连接")
#                 break
#             self.handle_classifications_data(data, address, data_handler)
#
#     def handle_client(self, client_socket, address):
#         data_handler = DataHandler(self.db_connection_pool)
#
#         data_frame_thread = threading.Thread(target=self.receive_data_frame,
#                                              args=(client_socket, address, data_handler))
...
class Server:
    ...
    def start(self):
        ...
        while True:
            client_socket, address = self.server_socket.accept()
            print(f"与 {address[0]}:{address[1]} 建立连接")
